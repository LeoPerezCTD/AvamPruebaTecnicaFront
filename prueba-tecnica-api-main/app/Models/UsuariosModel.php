        ...
        if($id == null){
            $sql = "SELECT -- superusuario
                ...
                -- 	usuarios propiedades.
                    left join (
                        ...
                        on uspr.usuario_id = us.usuario_id
                -- usuarios administradores.
                    left join (
                        ...
                        FROM
                            usuarios us
                        inner join usuarios_administrador uad
                            on uad.usuario_id = us.usuario_id
                            and uad.usuarioadmin_estado = 1
                        inner join complejos co
                            on co.complejo_id = uad.complejo_id
                            and co.complejo_estado = 1
                    ) as usad
                        on usad.usuario_id = us.usuario_id
                    inner join complejos co
                        on (co.complejo_id = usad.complejo_id
                        or co.complejo_id = uspr.complejo_id)
                    inner join usuarios_empresas uem
                        on uem.usuario_id = ?
                        and uem.empresa_id = co.empresa_id
                        and uem.usuarioempresa_estado = 1
                inner join personas pe
                    on pe.persona_id = us.persona_id
                    and pe.persona_estado = 1
                inner join perfiles_usuarios pu
                ...
                inner join (
                    select
                        ...
                        uad.usuario_id
                    from
                        usuarios_administrador uad
                    inner join complejos co
                        on co.complejo_id = uad.complejo_id
                    where uad.usuario_id = ?
                ) as tmp 
                    on tmp.complejo_id = pr.complejo_id
                inner join personas pe
                    on pe.persona_id = us.persona_id
                    and pe.persona_estado = 1
                inner join perfiles_usuarios pu
                ...
                inner join juntas ju
                    on ju.junta_id = uj.junta_id
                    and ju.junta_estado = 1
                inner join complejos co
                    on co.complejo_id = ju.complejo_id
                    and co.complejo_estado = 1
                inner join personas pe
                    on pe.persona_id = us.persona_id
                    and pe.persona_estado = 1
            ...
            $sql = "SELECT
                        ...
                        from (
                            ...
                            where pro.usuario_id = us.usuario_id
                        ) as tmp) as propiedades
                    from 
                        usuarios us
                    inner join personas pe
                        on pe.persona_id = us.persona_id
