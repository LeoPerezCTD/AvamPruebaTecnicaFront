...
class UsuariosEmpresasController extends MyController{
    private $model;
    public  $appModel;
    private $endpoint   = 'configuracion/usuarios-empresas';
    
    function __construct(){
        $this->model    = new UsuariosEmpresasModel();
            ...
            switch($action){
                case 'getParamsUpdate':
                    try{
                        ...
                        return $this->returnError('Error al consultar usuarios o empresas');
                    }
                break;
                // case 'getEdificios':
                //     $complejo   = $request->input('complejo_id');
                //     return $this->getEdificios($complejo);
                // break;
                default:
    ...
    public function show($id){
        ...
        if($permission){
            try{
                ...
                return $this->returnData($result,'No se obtuvo resultado de usuario de empresa');
            }catch(Exception $e){
                return $this->returnError('Error al obtener información de usuario de empresa');
            }
        }else{
            return $this->notPermission();
        }
