...
use App\Models\Products\QuoteDetailModel;
use App\Models\Products\QuoteModel;
use Illuminate\Http\Request;
use Exception;

class QuoteController extends MyController{
    private $model;
                ...
                default:
                    try{
                        ...
                        foreach($result as $key => $val){
                            $result[$key]->quote_detail = json_decode($val->quote_detail);
                        }
                    }catch(Exception $e){
                        return $this->returnError("0003"); // se produjo un error
                    }
    ...
    public function store(Request $request){
        ...
        if($permission){
            try{
                ...
                $result = $this->model->insertDataWeb($objData, $user);
                if($result === false){
    ...
    public function show($id){
        ...
        if($permission){
            try{
                ...
                $result[0]->quote_detail = json_decode($result[0]->quote_detail);
